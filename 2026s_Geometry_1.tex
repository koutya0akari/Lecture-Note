...
\section{第2回 : (2026/04/21)}

\subsection{距離関数と距離空間}
...
\section{第3回 : (2026/04/28)}

\subsection{平面 $\R^2$ 上の点列}
\begin{thm}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=1.2, >=Stealth, line cap=round, line join=round]
    ...
    \fill (O) circle (2pt);
    \node[below left] at (O) {$x=(0,0,0)$};

    \coordinate (P) at ({\R*cos(35)},{\R*sin(35)});
    \draw[thick,->] (O) -- (P);
    \node[midway, above right] {$\varepsilon$};

    \node at (0,2.9) {$U_{d_3}(x;\varepsilon)$};
    \node at (3.3,2.3) {$(\mathbb{R}^3,d_3)$};
